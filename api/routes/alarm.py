...
from schemas.alarm import AlarmCreate, AlarmUpdate, AlarmInDB

router = APIRouter()

@router.post("/alarm", response_model=AlarmInDB)
def create_alarm(alarm: AlarmCreate, db: Session = Depends(get_db)):
    try:
        created_alarm = crud_alarm.insert(db=db, id=alarm.id, alarm_time=alarm.alarm_time, repeat_day=alarm.repeat_day, light_color=alarm.light_color, alarm_status=alarm.alarm_status, user_id=alarm.user_id)
...
@router.get("/alarm/{id}", response_model=AlarmInDB)
...
@router.get("/alarms/user/{user_id}", response_model=List[AlarmInDB])
...
@router.put("/alarm/{id}", response_model=AlarmInDB)
